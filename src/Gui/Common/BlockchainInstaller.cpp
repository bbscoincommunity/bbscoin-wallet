...
BlockchainInstaller::BlockchainInstaller(QObject* _parent) : QObject(_parent), m_blockIndexesFileName("blockindexes.dat"), m_blocksFileName("blocks.dat"),
  m_dotoricoinDir(Settings::instance().getDataDir().absolutePath()), m_applicationDir(QDir::current()) {
}
...
void BlockchainInstaller::exec() {
  ...
  if (!checkIfDotoriCoinBlockchainExists()) {
    installBlockchain();
    return;
  }

  quint64 currentHeight;
  quint64 dotoricoinHeight;
  if (!checkIfBlockchainOutdated(currentHeight, dotoricoinHeight)) {
    return;
  }
  ...
  QuestionDialog dlg(tr("Blockchain installation"), QString(questionStringTemplate).arg(dotoricoinHeight).arg(currentHeight), nullptr);
...
bool BlockchainInstaller::checkIfDotoriCoinBlockchainExists() const {
  return m_dotoricoinDir.exists() && m_dotoricoinDir.exists(m_blocksFileName);
}
...
bool BlockchainInstaller::checkIfBlockchainOutdated(quint64& _sourceHeight, quint64& _targetHeight) const {
  ...
  QFile targetBlockIndexesFile(m_dotoricoinDir.absoluteFilePath(m_blockIndexesFileName));
...
QFileInfo BlockchainInstaller::dotoricoinBlockchainInfo() const {
  return QFileInfo(m_dotoricoinDir.absoluteFilePath(m_blocksFileName));
}
...
void BlockchainInstaller::installBlockchain() {
  ...
  Q_EMIT copyFileSignal(m_applicationDir.absoluteFilePath(m_blockIndexesFileName),
    m_dotoricoinDir.absoluteFilePath(m_blockIndexesFileName));
  if (waitLoop.exec() != 0) {
    workerThread.quit();
    workerThread.wait();
    return;
  ...
  Q_EMIT copyFileSignal(m_applicationDir.absoluteFilePath(m_blocksFileName), m_dotoricoinDir.absoluteFilePath(m_blocksFileName));
  if (waitLoop.exec() != 0) {
    workerThread.quit();
    workerThread.wait();
    return;
